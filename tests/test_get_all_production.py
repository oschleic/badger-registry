...
def test_vault_promotion_permissions(registry, vault, vault_one, vault_two, vault_three, gov, devGov):
  ...
  registry.promote("v1", vault_one, 0, "AAA", {"from": devGov})

  ## Gov can promote to anything
  registry.promote("v2", vault_two, 1, "BBB", {"from": gov})  
  registry.promote("v1", vault_three, 2, "CCC", {"from": gov}) 

  assert registry.getFilteredProductionVaults("v1", 0) == [[vault_one], ["AAA"]]
  assert registry.getFilteredProductionVaults("v2", 1) == [[vault_two], ["BBB"]]
  assert registry.getFilteredProductionVaults("v1", 2) == [[vault_three], ["CCC"]]
  
  result = registry.getProductionVaults()[0]
